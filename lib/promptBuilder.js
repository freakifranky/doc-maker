...
- Use ## for main section headings (e.g. ## Current Situation).
- Use ### for sub-section headings (e.g. ### Objective).
...
#### Analytics / Clickstream Requirements
(Group by P0, P1, P2)
#### Rollout Strategy
...
`.trim();
  ...
  // Release Note
  // =========================
  if (docType === "releaseNote") {
    return `
You are writing a release note or feature announcement that will be shared in a team chat (i.e. Slack, Lark).

Adapt the tone based on the input:
- If the audience mentions "alpha", "testers", "internal", or "testing" → write as an internal alpha release note. Tone: team-oriented, asks for feedback, includes testing steps.
- If the audience mentions "all hands", "company", "customers", or no specific audience → write as a feature announcement. Tone: celebratory, highlights impact, less focus on testing steps.
- When in doubt, lean toward internal/team tone.

Structure (use emoji headers):

## 🚀 Release Note: <Title> (<Version>)

A 1-2 sentence intro: who this is for and what they should know.

## ✨ What's New
One paragraph on the headline change and why it matters.

## 🔧 What Changed
Key updates as bullet points. Be specific — name components, screens, behaviors.

## 📲 How to Test (include if audience is testers/internal/alpha)
Numbered steps to try the feature. Include version numbers, locations, setup steps.

## ⚠️ Notes (include only if there are caveats)
Caveats: things still in progress, known issues, design not finalized.

## 🙏 Credits (include if provided)
Shout out the team. Keep it genuine.

Tone guidelines:
- Friendly, like posting in a team channel — not corporate
- Use emoji for section headers
- Be specific about what changed — name components, screens, versions
...
**Title:** ${a.rnTitle || "-"}
**Audience:** ${a.rnAudience || "-"}
**Version:** ${a.rnVersion || "-"}
**What's New:** ${a.rnWhatsNew || "-"}
**What Changed:** ${a.rnWhatsChanged || "-"}
**How to Test:** ${a.rnHowToTest || "-"}
**Notes:** ${a.rnNotes || "-"}
**Credits:** ${a.rnCredits || "-"}
`.trim();
  }
